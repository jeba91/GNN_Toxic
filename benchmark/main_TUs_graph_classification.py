...
def train_val_pipeline(MODEL_NAME, DATASET_NAME, params, net_params, dirs):
    ...
    try:
        for split_number in range(10):
            ...
            drop_last = True if MODEL_NAME == 'DiffPool' else True
            ...
            with tqdm(range(params['epochs'])) as t:
                for epoch in t:
                    ...
                    if best_test_acc[split_number] < epoch_test_acc:
                        best_test_acc[split_number] = epoch_test_acc
                        files = glob.glob(ckpt_dir + "/best/*.pkl")
                        for file in files:
                            os.remove(file)
                        torch.save(model.state_dict(), '{}.pkl'.format(ckpt_dir + "/best/" + str(epoch) + "_" + str(epoch_test_acc)))

                    if best_train_acc[split_number] < epoch_train_acc:
                        best_train_acc[split_number] = epoch_train_acc
    ...
    print("""\n\n\nFINAL RESULTS\n\nBEST TR ACCURACY averaged: {:.4f} with s.d. {:.4f}"""          .format(np.mean(np.array(best_test_acc))*100, np.std(best_test_acc)*100))
    print("\nAll splits Test Accuracies:\n", best_test_acc)
    print("""\n\n\nFINAL RESULTS\n\nBEST TE ACCURACY averaged: {:.4f} with s.d. {:.4f}"""          .format(np.mean(np.array(best_train_acc))*100, np.std(best_train_acc)*100))
    print("\nAll splits Test Accuracies:\n", best_train_acc)
    ...
    print("""\n\n\nFINAL RESULTS\n\nTRAIN ACCURACY averaged: {:.4f} with s.d. {:.4f}"""          .format(np.mean(np.array(avg_val_acc))*100, np.std(avg_val_acc)*100))
    print("\nAll splits Train Accuracies:\n", avg_val_acc)
...
def main():
    ...
    # TUs
    net_params['in_dim'] = dataset.train[0].graph_lists[0].ndata['feat'][0].shape[0]
